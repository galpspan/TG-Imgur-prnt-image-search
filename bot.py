...
"""
    ...
    async def stop(self, update: Update, context: CallbackContext):
        ...
        if session and session.get("task"):
            ...
            logger.info(f"Пользователь {user_id} попытался использовать /stop, но поиск не был начат.")
    ...
    async def get_imgur_images(self, update: Update, context: CallbackContext):
        ...
        if user_id in self.sessions:
            ...
            if prev_session.get("task"):
                ...
                logger.info(f"Предыдущий поиск пользователя {user_id} был отменён перед началом нового")
        ...
        status_msg = await update.message.reply_text(
            f"🔍 Ищу {count} изображений на Imgur длиной {length}...\nНайдено: 0/{count}\nПроверено: 0"
        )
        ...
        async def update_status():
            ...
            await status_msg.edit_text(
                f"🔍 Ищем {count} изображений на Imgur...\n"
                f"Найдено: {found}/{count}\n"
                f"Проверено: {analyzed}\n"
                f"Время: {elapsed // 60}м {elapsed % 60}с"
            )
    ...
    async def get_prnt_images(self, update: Update, context: CallbackContext):
        ...
        logger.info(f"Prnt.sc поиск пользователя {user_id} начат. Длина: {length}, количество: {count}")

        status_msg = await update.message.reply_text(
            f"🔍 Ищу {count} изображений на prnt.sc...\nНайдено: 0/{count}\nПроверено: 0"
        )
        ...
        async def update_status():
            ...
            elapsed = int(time.time() - start_time)
            await status_msg.edit_text(
                f"🔍 Ищем {count} изображений на prnt.sc...\n"
                f"Найдено: {found}/{count}\n"
                f"Проверено: {analyzed}\n"
                f"Время: {elapsed // 60}м {elapsed % 60}с"
            )
                ...
                for result in results:
                    ...
                    if img_url:
                        ...
                        if ext[1]:
                            ...
                            try:
                                if ext[1] == 'gif':
                                    await update.message.reply_animation(animation=img_url, caption=caption, parse_mode="Markdown")
                                else:
